         ...
         if(functions.includes(node.expression.memberName) || functions.includes(node.expression.expression.memberName)){
          ...
          if (node.expression.expression && functions.includes(node.expression.expression.memberName)){
           ...
           const valueArguments = source.slice(node.expression.arguments.range[0], node.expression.arguments.range[1]+1);
              if(node.expression.expression.memberName === 'call'){
                ...
                mutations.push(new Mutation(file, start, end + 1, address+'.call'+'{'+valueArguments+'}'+'('+callArguments+')'))
                mutations.push(new Mutation(file, start, end + 1, address+'.staticcall'+'{'+valueArguments+'}'+'('+callArguments+')'))  

              }else if (node.expression.expression.memberName === 'staticcall'){
                mutations.push(new Mutation(file, start, end + 1, address+'.call'+'{'+valueArguments+'}'+'('+callArguments+')'))
                mutations.push(new Mutation(file, start, end + 1, address+'.delegatecall'+'{'+valueArguments+'}'+'('+callArguments+')'))  
              }
            }
          }
          //Send, transfer, and call methods with no Ether value or Gas value supplied - ex: call("")
            ...
            if (node.expression.memberName == 'call'){
              mutations.push(new Mutation(file, start, end + 1, address+'.delegatecall'))
              mutations.push(new Mutation(file, start, end + 1, address+'.staticcall'))       
            }
            //delegatecall
            else if (node.expression.memberName == 'delegatecall'){
              mutations.push(new Mutation(file, start, end + 1, address+'.call'))
              mutations.push(new Mutation(file, start, end + 1, address+'.staticcall'))       
            }
            //staticcall
            ...
            //send
              ...
              mutations.push(new Mutation(file, start, end + 1, address+'.transfer'))
              ...
              mutations.push(new Mutation(file, start, end + 1, address+'.send'))
              start = node.range[0];
              end = node.range[1];
              replacement = address+'.call{value: '+arg;
              if(subdenomination)
